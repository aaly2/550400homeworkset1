...
\newline
Because we are modeling the match using a biased coin, we find that the probability of winning a set for a single player is unaffected if the number of matches is even. This makes sense because Player A and B get the same advantage.When the number of matches is even then the equation equals one, because the numerator the denominator will be equal. They cancel out and this gives the a ratio of one. However, if the number of matches is odd, then the player that starts first will gain a greater advantage, since that player will start first more times than the other player. The numerator will not equal the denominator. When simplified, the equation will not equal one. \newline
\newline
